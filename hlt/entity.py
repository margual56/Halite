...
class Entity:
    ...
    def getAngle(self, target, closest, cleareance):
        """
        :param Entity target: The entity to which you will navigate
        :param Entity closest: The closest obstacle
        :param float cleareance: cleareance percentaje for the planet radius
        :return double: The +-angle of the tangent
        :rtype: str
        """
        r = (closest.radius + self.radius)  # *(1 + cleareance)
        d = target.calculate_distance_between(closest)
        return math.acos(d / math.sqrt(d * d + r * r))

    def intersects(self, other):
        """
        :param Entity other: The instance to check the collision against
        :return bool: Wether it is intersecting other or not
        :rtype: bool
        """
        d = self.calculate_distance_between(other)
...
class Planet(Entity):
    ...
    def ship_is_docked(self, ship):
        for docked in self.all_docked_ships():
            if docked == ship:
                return True
    ...
    def getUndock(self):
        return self.toUndock

    def addUndock(self):
        self.toUndock += 1

    def resetUndock(self):
        self.toUndock = 0
...
class Ship(Entity):
    ...
    '''def navigate_ship(self, target, game_map, speed, cleareance):
        """
        :param Entity target: The entity to which you will navigate
        :param game_map.Map game_map: The map of the game, from which obstacles will be extracted
        :param int speed: The (max) speed to navigate. If the obstacle is nearer, will adjust accordingly.
        :param Entity[] allObstacles: All the obstacles to avoid
        :param float cleareance: cleareance percentaje for the planet radius
        :return string: The command trying to be passed to the Halite engine or None if movement is not possible within max_corrections degrees.
        :rtype: str
        """
        allObstacles = game_map.obstacles_between(self, target, Ship)

        if len(allObstacles) == 0:
            distance = Position.dist(Position(self.x, self.y), target)
            angle = math.atan2(-self.y + target.y, -self.x + target.y)
            speed = speed if (distance >= speed) else distance
            return self.thrust(speed, angle)

        i = 0
        while allObstacles[i].intersects(self):
            i += 1
            if i >= len(allObstacles):
                distance = Position.dist(Position(self.x, self.y), target)
                angle = math.atan2(-self.y + target.y, -self.x + target.y)
                speed = speed if (distance >= speed) else distance
                return self.thrust(speed, angle)

        closest = allObstacles[i]

        a = closest.getAngle(target, closest, cleareance)
        if a > 0.383972:
            a *= 1.2

        d = self.calculate_distance_between(target)
        tmpA = -closest.angleBetween(target)

        p1 = Position(math.cos(tmpA + a) * d + target.x, math.sin(tmpA + a) * d + target.y)
        p2 = Position(math.cos(tmpA - a) * d + target.x, math.sin(tmpA - a) * d + target.y)

        dist = target.calculate_distance_between(closest) * 1.1
        p21 = Position(math.cos(tmpA + a) * dist + target.x, math.sin(tmpA + a) * dist + target.y)
        p22 = Position(math.cos(tmpA - a) * dist + target.x, math.sin(tmpA - a) * dist + target.y)

        if self.lastTarget is None:
            if Position.dist(Position(self.x, self.y), p21) < Position.dist(Position(self.x, self.y), p22):
                nextPoint = p21
            else:
                nextPoint = p22
        else:
            if Position.dist(self.lastTarget, p21) < Position.dist(self.lastTarget, p22):
                nextPoint = p21
            else:
                nextPoint = p22

        self.lastTarget = nextPoint

        distance = Position.dist(Position(self.x, self.y), nextPoint)
        angle = math.atan2(-self.y + nextPoint.y, -self.x + nextPoint.y)
        speed = speed if (distance >= speed) else distance
        return self.thrust(speed, angle)'''
...
class Position(Entity):
    ...
    def dist(self, other):
        """
                ...
                :param Position other: The target to get distance to.
                :return: distance
                :rtype: float
                """
